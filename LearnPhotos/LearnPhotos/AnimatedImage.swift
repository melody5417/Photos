...
//  LearnPhotos
//
//  Created by yiqiwang(王一棋) on 2018/3/28.
//
...
class AnimatedImage {
    ...
    init(source: CGImageSource) {
        ...
        if let imageProperites = CGImageSourceCopyProperties(source, nil) as? [String: AnyObject] {
            loopCount = AnimatedImage.loopCountForProperties(properties: imageProperites)
        } else {
        ...
        var delayTimes = [Double](repeating: (1.0/3.0), count: frameCount)
        var totalDuration: Double = 0.0
        for index in 0..<frameCount {
            if let imageProperties = CGImageSourceCopyPropertiesAtIndex(source, 0, nil) as? [String: AnyObject] {
    ...
    static func frameDelayForProperties(properties: [String: AnyObject]) -> Double? {
        ...
        if let delay = (gifDictionary[kCGImagePropertyGIFDelayTime as String] as? NSNumber)?.doubleValue {
            if delay > 0.0 {
                return delay
            }
        }
